...
Claude Code reads `CLAUDE.md` automatically at the start of every session. That file contains your company overview, ICP, signals, personas, pricing, and current priorities. Once it's filled in, you can run any task with a one-line prompt — no re-explaining your context each time.

```
Read skills/account-research/SKILL.md and research squareup.com
```
...
│   ├── pricing.md                     ← Pricing models, packaging types, competitor comparisons
...
```bash
git clone https://github.com/heyudo5-max/spark-kit
cd spark-kit
claude .
```
...
When it's done, it shows you what was filled and what was inferred — then offers a 3-minute refinement pass (6 questions) to replace inferred fields with your actual win patterns, ACV range, and signals. Optional. The repo works either way.
